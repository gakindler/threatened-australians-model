...
here::here()
here::i_am("writing/TA_theory/Kindler_2023_TA_theory.qmd")
```

```{r}
quad_data <- tibble(
  title = c("TA", "ALA", "My Backyard"),
  motives = c(7.5, 2.5, 7.5),
  audience = c(7.5, 2.5, 2.5)
)
...
plot <- ggplot(quad_data, aes(x = motives, y = audience, label = title)) +
  coord_fixed() +
  ...
  # annotate("text", x = 2.5, y = 2.5, alpha = 0.35, label = "X") +
  # annotate("text", x = 2.5, y = 7.5, alpha = 0.35, label = "Y") +
  # annotate("text", x = 7.5, y = 2.5, alpha = 0.35, label = "Z") +
  # annotate("text", x = 7.5, y = 7.5, alpha = 0.35, label = "W")


plot

ggsave(here("output/quadrant.pdf"), width = 15, height = 15, units = "cm")
```
