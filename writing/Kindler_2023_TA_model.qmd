...
At least three users expressed desire for a list of all threatened species in each electoral division that would allow them to search or quickly scan as opposed to click to cycle through species one at a time.
...
Multiple users expressed a discrepancy between their knowledge of which threatened species are local to them and what was presented to them. This observation from users highlights the obselescence of the EPBC Act data.
...
* "Brilliant electorate based platform to connect people to biodiversity in their area, and more activate them into electorate level mobilisations [sic]"
...
When users were asked if they have any suggestions for ways that we might improve the project, responses regularly mentioned expanding the user interface to include a high-level navigation and downloadable list of threatened species in each electoral division. For example, "It would be good to have a list of the animals/plants first with links rather than have to click through". A user suggested "I think it would be extremely useful to add actions for kids to take in the "what you can do" part".
...
There were a total of `r data_nonspatial$engag_title_clean |> summarise(sum = sum(event_count))` events on the Electoral District page.

* How many events occurred on the How-to-help page?

There were a total of `r data_nonspatial$help_title_clean |> summarise(sum = sum(event_count))` events on the How to Help page.
